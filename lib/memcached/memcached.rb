...
### Setters
  ...
  def set(key, value, ttl=@default_ttl, encode=true, flags=FLAGS)
    value, flags = options[:encoder].encode(key, value, flags) if encode
    begin
      check_return_code(
  ...
  def cas(key, ttl=@default_ttl, encode=true, flags=FLAGS)
    ...
    value = options[:encoder].decode(key, value, flags) if encode
    value = yield value
    value, flags = options[:encoder].encode(key, value, flags) if encode
...
### Getters
    ...
    if keys.is_a? Array
      ...
      if encode
        hash.each do |key, value_and_flags|
          hash[key] = options[:encoder].decode(key, *value_and_flags)
        end
      end
      hash
    else
      ...
      check_return_code(ret, keys)
      encode ? options[:encoder].decode(keys, value, flags) : value
    end
  rescue => e
    tries ||= 0
    raise unless tries < options[:exception_retry_limit] && should_retry(e)
  ...
  def get_from_last(key, encode=true)
    ...
    check_return_code(ret, key)
    encode ? options[:encoder].decode(key, value, flags) : value
  end
