...
extern unsigned int memcached_server_count(m_obj INPUT);
extern m_ret memcached_server_add(m_obj INPUT, char* hostname, unsigned int port= 11211);
...
// Incr and decr
extern m_ret memcached_increment(m_obj INPUT, char *key, size_t key_length, unsigned int offset, uint64_t value);
extern m_ret memcached_decrement(m_obj INPUT, char *key, size_t key_length, unsigned int offset, uint64_t value);

// Getting data
extern char* memcached_get(m_obj INPUT, char *key, size_t key_length, size_t *value_length, uint32_t flags, m_ret* error);
extern m_ret memcached_mget(m_obj INPUT, char **keys, size_t *key_length, unsigned int number_of_keys);
